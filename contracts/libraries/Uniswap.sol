...
library Uniswap {
    ...
    function deposit(Position memory position, uint128 liquidity)
        internal
        returns (uint256 amount0, uint256 amount1)
    {
        if (liquidity != 0) {
            (amount0, amount1) = position.pool.mint(
                address(this),
                position.lower,
                position.upper,
                liquidity,
        ...
        )
    {
        ...
        (uint128 liquidity, , , uint128 earnable0, uint128 earnable1) = info(
            position
        );
        (uint256 burnable0, uint256 burnable1) = amountsForLiquidity(
            position,
            sqrtPriceX96,
            liquidity
        );
    ...
    function liquidityForAmount0(Position memory position, uint256 amount0)
        internal
        pure
        returns (uint128)
    {
    ...
    function liquidityForAmount1(Position memory position, uint256 amount1)
        internal
        pure
        returns (uint128)
    {
