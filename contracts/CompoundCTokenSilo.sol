...
contract CompoundCTokenSilo is ISilo {
    ...
    function withdraw(uint256 amount) external override {
        if (amount == 0) return;
        uint256 cAmount = 1 +
            FullMath.mulDiv(amount, 1e18, ICToken(cToken).exchangeRateStored());

        require(
            ICToken(cToken).redeem(cAmount) == 0,
    ...
    /// @inheritdoc ISilo
    function shouldAllowRemovalOf(address token)
        external
        view
        override
        returns (bool shouldAllow)
    {
